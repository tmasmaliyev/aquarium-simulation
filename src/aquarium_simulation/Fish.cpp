...
Fish::Fish(std::string _s, glm::vec3 initialPosition, glm::vec3 initialRotation, glm::vec3 initialScaling)
{
	...
	if (_s == "norandom")
	{
		...
		y = initialPosition.y;
		z = initialPosition.z;
		rx = initialRotation.x;
		ry = initialRotation.y, rz = initialRotation.z;
		sx = initialScaling.x;
		sy = initialScaling.y;
		sz = initialScaling.z;
		Velocity = 0.01f;
	...
	{
		x = random(-MAX_X, MAX_X);
		y = random(1.0f, MAX_Y);
		z = random(-MAX_Z, MAX_Z);
		rx = random(0.0f, 360.0f);
		ry = random(0.0f, 360.0f);
		rz = 0.0f;
	...
	std::string path_obj = "./assets/Models/" + my_fishes[number_of_model] + ".obj";
	...
	std::string path_tex = "./assets/Models/" + my_fishes[number_of_model] + ".jpg";
