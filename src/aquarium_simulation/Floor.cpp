...
Floor::Floor() : Length(LENGTH), Width(WIDTH)
{
	...
	{
	case sand:
		data = stbi_load("./assets/Models/sand.jpg", &TextureWidth, &TextureHeight, &nrChannels, 0);
		break;
	case sand2:
		data = stbi_load("./assets/Models/gravel.jpg", &TextureWidth, &TextureHeight, &nrChannels, 0);
		break;
	default:
		break;
	}
